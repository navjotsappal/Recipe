...
$userId=$recipeId=$recipeName=$selectCategory=$otherCategory=$preparationTime=$cookTime=$servings=$rating=$imageUpload=$video=$youtubeId="";
$ingredients=$directions=$description=$notesComments=$collaborateEmail=$shareEmail=$shareTime=$recipeCreationTime="";

$target_image_path ="";
$oldImagePath=$oldCollaborateEmail=$oldShareEmail=$oldShareTime=$oldRecipeCreationTime=$oldToken=$oldRating="";
...
$userId=$_SESSION['uid'];
$recipeId=$_REQUEST['q'];
$fetchRecipe=readRecipe($errorMessage_permissionDenied);
//echo ">>>".$errorMessage_permissionDenied;

if($fetchRecipe!=false)
{
    $row=$fetchRecipe;
    ...
    $oldImagePath=$row['imageURL'];
    $target_image_path=$row['imageURL'];
    $_POST['fileToUpload']=$oldImagePath;

    $video=$row['videoURL'];
    if(!empty($video))
    {
        list($url,$youtubeId)=explode("embed/",$video); // put first part of youtube address, source:http://php.net/manual/en/function.explode.php
    ...
    $oldCollaborateEmail=$row['collaborateEmail'];

    $shareEmail=$row['timeShareEmail'];
    $oldShareEmail=$row['timeShareEmail'];

    $shareTime=$row['timeShareTime'];
    $oldShareTime=$row['timeShareTime'];

    $oldRecipeCreationTime=$row['recipeCreationTime'];
    $oldToken=$row['timeShareToken'];
...
function readRecipe(&$errorMessage_permissionDenied){
    ...
    try
    {
        ...
        if($row===false){                                     //email exists already, source: http://thisinterestsme.com/php-user-registration-form/
            ...
            $errorMessage_permissionDenied="Request rejected! You do not have permission to update this recipe!";
            return false;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   // var_dump($_POST);
    ...
    if(strcmp($target_image_path,$oldImagePath)==0)
    {
        $target_image_path=$oldImagePath;
    }
    else{
    ...
    else if(isset($_POST['star'])==false)
    {
        ...
        $checkValidations=false;
        $errorMessage_rating="Php error: Please rate your recipe .";
    ...
    if(!empty($collaborateEmail)|| !empty($shareEmail)){
        ...
        else if (strcmp($collaborateEmail,$_SESSION["email"])==0)
        {
            ...
            //echo "BBBBBB ".$shareEmail." AAAAA ".$_SESSION['email'];
        }
...
if((isset($_POST['submit'])) and $checkValidations==true) {
    ...
    try
    {
        ...
        $sql = "SELECT * FROM recipe WHERE recipeId=:recipeId and userId=:userId";  //preparing the SQL statement, source: http://thisinterestsme.com/php-user-registration-form/
        $statement = $connection->prepare($sql);
        $statement->bindValue(':recipeId', $recipeId);     //binding recipeName to prepare statement, then execute the statement. source: http://thisinterestsme.com/php-user-registration-form/
        $statement->bindValue(':userId', $userId);
        $statement->execute(); //Fetch row.
      ...
      //  var_dump($row);
        ...
        if($row!==false)
        {
            ...
            if(strcmp($collaborateEmail,$oldCollaborateEmail)==0)
            {
                $collaborateEmail=$oldCollaborateEmail; // do not send collaborate email if old and new collab emails are same
            }
            ...
            if(strcmp($shareEmail,$oldShareEmail)==0)
            {
                $shareEmail=$oldShareEmail;
                $token=$oldToken;
                $recipeCreationTime=$oldRecipeCreationTime;
                if(strcmp($shareTime,$oldShareTime)==0)
                {
                    $shareTime=$oldShareTime;
                }
                else  // send new email with new sharing time and old share email, old token and old recipe creation time
                {
                    if(sendTimeShareEmail($token,$shareEmail,$shareTime,$recipeName)){
                        ...
                        $errorMessage_shareEmail="Sorry! We couldn't send an email with updated share time.";
                        $errorMessage_shareEmailCSS="border-color:#DE1643";
            ...
            else
            {
                ...
                {
                    $token=md5(mt_rand());  //create new token for new share email
            ...
            {
                $statement = $connection->prepare("Update recipe set recipeName= :recipeName,category=:category,preparationTime=:preparationTime,cookTime=:cookTime,servings=:servings,rating=:rating,imageURL=:imageURL,videoURL=:videoURL,ingredients=:ingredients,directions=:directions, description=:description, notesComments=:notesComments, collaborateEmail=:collaborateEmail, timeShareEmail=:timeShareEmail, timeShareTime =:timeShareTime, timeShareToken=:timeShareToken, recipeCreationTime =:recipeCreationTime where userId=:userId and recipeId=:recipeId"); //// bind input values to prepare statement, then execute the statement,  source: http://thisinterestsme.com/php-user-registration-form/
                                    ...
                                    <div class=\"modal-dialog modal-dialog-centered\" role=\"document\">
                                      <div class=\"modal-content\">
                                          ...
                                          <div class=\"modal-body\">
                                             <label style=\"font-size: 1em;\"> Your recipe has been updated successfully.</label>
                                             <br><br>
                                            <label style=\"font-size: 1em;\">  Please click on <b>My Recipes</b> to view your recipes.</label>
                                          </div>
                                          <div class=\"modal-footer\">
                                             <button onclick=\"window.location='myRecipes.php?selectCategory=$tempSelectCategory&submit=VIEW+RECIPES';\" style=\"background-color: deepskyblue; color: white;\" type=\"button\" class=\"btn btn-secondary\" data-dismiss=\"modal\">My Recipes</button>
                                          </div>
                                     </div>
                                  </div>
                               </div>
    ...
    catch(PDOException $e)       //catching exception, source : https://www.w3schools.com/php/php_mysql_insert.asp
    {
        ...
        $userMessageRecipeCreationFail=" 1) Fail to fetch and update recipe data into the database  2) Fail to set up the database connection";
...
<?php
         if(!empty($errorMessage_permissionDenied))   // user tries to access recipe that does not belong to user, deny permission
         {
             ?>
                   <div class="container enclosedContainer">
                      <div class="container containerWidth">                                  <!--outer div for SignUp, source: https://www.w3schools.com/css/tryit.asp?filename=trycss_form_responsive-->
                         <div class="container">
                            <div class="row text-center" style="background-color:#f8d7da !important; margin-top: 1em !important;">
                                <div class="customAlert">
                                    <h3 class="errorColor"><span class="glyphicon glyphicon-ban-circle errorColor"></span><strong> Permission denied !</strong></h3>
         ...
         else
         {
             ...
             <div class="container enclosedContainer">
                 <div class="container containerWidth">                                  <!--outer div for SignUp, source: https://www.w3schools.com/css/tryit.asp?filename=trycss_form_responsive-->
                     <form method="post" enctype="multipart/form-data" "<?php echo htmlentities($_SERVER["PHP_SELF"] . '?'.http_build_query($_GET));?>">
                         ...
                         <div class="container">
                             ...
                             <div class="row"  >  <!-- source:  https://www.w3schools.com/css/tryit.asp?filename=trycss_form_responsive */-->
                                 ...
                                 <br>
                                 <h1 class="titleHeading" style="padding: 0 0.5em; margin-bottom: -0.25em;"><b> Update Recipe</b></h1>
                                 <br>
                                 <br>
                             ...
                             <div class="row">                                <!-- source:  https://www.w3schools.com/css/tryit.asp?filename=trycss_form_responsive */-->
                                 <div class="columnOne">                 <!--for text input in right column, source:  https://www.w3schools.com/css/tryit.asp?filename=trycss_form_responsive */-->
                                     ...
                                     <div>
                                         ...
                                         <select class="selectCategory" name="selectCategory" size="1" id="selectCategory"  onchange="fnSelectCategory();" style="<?php echo $errorMessage_selectCategoryCSS;?>">
                                            ...
                                            <?php
                                                if((strcmp($selectCategory,"indian")!=0) && (strcmp($selectCategory,"mexican")!=0)&&(strcmp($selectCategory,"chinese")!=0)&&(strcmp($selectCategory,"italian")!=0)&&(strcmp($selectCategory,"french")!=0)&& (strcmp($selectCategory,"greek")!=0)&&(strcmp($selectCategory,"spanish")!=0)&&(strcmp($selectCategory,"thai")!=0)&&(strcmp($selectCategory,"german")!=0)&&(strcmp($selectCategory,"asian")!=0)&&(strcmp($selectCategory,"african")!=0)&&(strcmp($selectCategory,"southern")!=0))
                                                {
                                                    ?>
                                                    <option value='<?php echo $selectCategory;?>' <?php echo "selected='selected'"?>> <?php echo $selectCategory?></option>
                                     ...
                                     <div>
                                         ...
                                         <div title="Please rate your recipe" class="starRating">
                                             ...
                                             <input class="star starFive" onclick="validateRating();" type="radio" id="starFive" name="star" <?php if ((isset($_POST['star']) && (int)($_POST['star'])==5)||((int)($rating)==5)) echo "checked";?> value="5"/>  <!-- source: https://stackoverflow.com/questions/27082153/retain-radio-button-selection-after-submit-php-->
                                             <label class="star starFive" id="five" for="starFive"></label>

                                             <input class="star starFour" onclick="validateRating();" type="radio" id="starFour" name="star" <?php if ((isset($_POST['star']) && (int)($_POST['star'])==4)||((int)($rating)==4)) echo "checked";?> value="4"/>
                                             <label class="star starFour" id="four" for="starFour"></label>

                                             <input class="star starThree" onclick="validateRating();" type="radio" id="starThree" name="star" <?php if ((isset($_POST['star']) && (int)($_POST['star'])==3)||((int)($rating)==3)) echo "checked";?> value="3"/>
                                             <label class="star starThree" id="three" for="starThree"></label>

                                             <input class="star starTwo" onclick="validateRating();" type="radio" id="starTwo" name="star" <?php if ((isset($_POST['star']) && (int)($_POST['star'])==2)||((int)($rating)==2)) echo "checked";?> value="2"/>
                                             <label class="star starTwo" id="two" for="starTwo"></label>

                                             <input class="star starOne"  onclick="validateRating();"  type="radio" id="starOne" name="star" <?php if ((isset($_POST['star']) && (int)($_POST['star'])==1)||((int)($rating)==1)) echo "checked";?> value="1"/>
                                     ...
                                     <div>
                                         <label><span class="glyphicon glyphicon-picture"></span> Upload New Image *</label>
                                         <?php
                                           if(strcmp($target_image_path,$oldImagePath)==0)
                                           {
                                               $target_image_path=$oldImagePath;
                                               //echo $target_image_path.">>>>>>>>".$oldImagePath;

                                           }
                                         ?>
                                         <img class="img" src="<?php echo $target_image_path?>">
                                 ...
                                 <div class="columnOne">
                                     ...
                                     <div>
                                         ...
                                         <input type="email" id="shareEmailInput" placeholder="Please enter email (user@email.com) of the user." name="shareEmail"  value="<?php echo $shareEmail;?>" onfocusout="validateShareEmail();" style="<?php echo $errorMessage_shareEmailCSS;?>"> <br>
                                     ...
                                     <div>
                                         ...
                                         <input style="margin-bottom: 0em;" type="number" id="shareTimeInput" pattern="[0-9]{1,3}" title="Only digits allowed !" min="1" max="365" placeholder="Only digits allowed. Adding user email will activate this textbox." name="shareTime"  value="<?php echo $shareTime;?>" onchange="validateShareTime();" style="<?php echo $errorMessage_shareTimeCSS;?>">
