...
try
{
    ...
    $sql = "SELECT recipeName,category,preparationTime,cookTime,servings,rating,imageURL,videoURL,ingredients,directions,description,notesComments,collaborateEmail,timeShareEmail,timeShareTime,recipeCreationTime FROM recipe WHERE timeShareToken  = :token";  //preparing the SQL statement, source: http://thisinterestsme.com/php-user-registration-form/
    $statement = $connection->prepare($sql);
    $statement->bindValue(':token', $fetchToken);     //binding recipeName to prepare statement, then execute the statement. source: http://thisinterestsme.com/php-user-registration-form/
    ...
    if($row===false){                                     //email exists already, source: http://thisinterestsme.com/php-user-registration-form/
        ...
        $errorMessage_permissionDenied="Request rejected! You do not have permission to view this recipe!";
    }

    else {

        $currentTime= time(); // source: https://stackoverflow.com/questions/4298059/time-subtract-mysql-and-php
        $recipeCreationTime=$row['recipeCreationTime'];

        $timeGapDifference=(int)($currentTime-$recipeCreationTime); // source: https://stackoverflow.com/questions/4298059/time-subtract-mysql-and-php
        $timeShareTime=(int)(((int)($row['timeShareTime']))*24*60*60);

        //echo $timeGapDifference." MMM ".$timeShareTime;
        if($timeGapDifference > $timeShareTime)
        {
            $errorMessage_permissionDenied="Sorry! Your time limit to access the recipe has expired!";
